...
class TestCreateInitialSettings:
    @pytest.fixture
    def mock_db(self):
        db = MagicMock()
    ...
    @pytest.mark.asyncio
    async def test_create_initial_settings_success(self, mock_db, mock_logger, user_id):
        db, collection = mock_db
        collection.find_one.return_value = None
        logger = mock_logger

        await create_initial_settings(db, logger, user_id)

        db.get_collection.assert_any_call("start_settings")
        collection.find_one.assert_awaited_with({"user_id": user_id})
        assert collection.insert_one.await_count == 1
        logger.info.assert_called()

    @pytest.mark.asyncio
    async def test_create_initial_settings_already_exists(
