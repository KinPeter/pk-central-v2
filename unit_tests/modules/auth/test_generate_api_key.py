...
class TestGenerateApiKey:
    ...
    @pytest.mark.asyncio
    async def test_returns_generate_api_key_response(self, request_obj, user):
        ...
        with patch(
            ...
            result = await generate_api_key(request_obj, user)

        assert isinstance(result, GenerateApiKeyResponse)

    @pytest.mark.asyncio
    async def test_returns_raw_key_in_response(self, request_obj, user):
        db_collection = AsyncMock()
        db_collection.insert_one = AsyncMock()
        request_obj.app.state.db.get_collection.return_value = db_collection
    ...
    @pytest.mark.asyncio
    async def test_response_includes_key_id(self, request_obj, user):
        db_collection = AsyncMock()
        db_collection.insert_one = AsyncMock()
        request_obj.app.state.db.get_collection.return_value = db_collection
    ...
    @pytest.mark.asyncio
    async def test_inserts_correct_document_into_db(self, request_obj, user):
        db_collection = AsyncMock()
        db_collection.insert_one = AsyncMock()
        request_obj.app.state.db.get_collection.return_value = db_collection
    ...
    @pytest.mark.asyncio
    async def test_hashed_key_is_stored_not_raw_key(self, request_obj, user):
        db_collection = AsyncMock()
        db_collection.insert_one = AsyncMock()
        request_obj.app.state.db.get_collection.return_value = db_collection
        ...
        with patch(
            ...
            await generate_api_key(request_obj, user)
    ...
    @pytest.mark.asyncio
    async def test_logs_info_on_success(self, request_obj, user):
        db_collection = AsyncMock()
        db_collection.insert_one = AsyncMock()
        request_obj.app.state.db.get_collection.return_value = db_collection
    ...
    @pytest.mark.asyncio
    async def test_raises_internal_server_error_on_db_failure(self, request_obj, user):
        db_collection = AsyncMock()
