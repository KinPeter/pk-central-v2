...
async def update_chore(
    request: Request, id: str, body: ChoreRequest, user: CurrentUser
) -> ActivitiesConfig:
    """
    ...
    try:
        ...
        result = await collection.update_one(
            {"user_id": user.id, "chores.id": id},
            {"$set": {"chores.$": chore_data}},
        )

        if result.matched_count == 0:
            logger.error(
                f"Failed to update chore {id} for user {user.id}, config not found"
            )
        ...
        logger.error(f"Error updating chore {id} for user {user.id}: {e}")
        raise InternalServerErrorException(
            "An error occurred while updating the chore: " + str(e)
        )
